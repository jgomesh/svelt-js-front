...
const addToCart = (quantity, product, cartInfo, userId, setCartInfo, setQuantity) => {
  ...
  if(data) {
    ...
    const productAlreadyExist = dataDecoded.sales_products.some((productExist) => {
    ...
    const prices = products.sales_products.map((product1) => Number(product1.price) * Number(product1.quantity));
    const totalPrice = addAllPrices(prices);
  ...
  const prices = cart.sales_products.map((product1) => Number(product1.price) * Number(product1.quantity));
  const totalPrice = addAllPrices(prices);
