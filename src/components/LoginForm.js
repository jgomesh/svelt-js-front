...
function LoginForm({ setLoading, userInfo, history, setUserInfo, loading, loginOpen, setLoginOpen }) {
  const [error, setError] = useState(false)
  return (
    <form className={`login__section__form${loginOpen? '' : '__hidden'}`} onSubmit={(target) => login(target, setLoading, userInfo, setError, history)}>
      ...
      {loading? 'Loading...' : (
        <div>
          <div className='login__section__form__container'>
            <button name='register button' onClick={(event) => redirect(event, history, '/register')}>
              Register
            </button>
