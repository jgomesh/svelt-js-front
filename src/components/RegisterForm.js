...
import RegisterErros from './RegisterErros';
...
function RegisterForm({ userInfo, setError, setRegistered, disabled, error, registered, setUserInfo, setDisabled, setLoginOpen } ) {
  const [ role, setRole ]= useState('register');
  const [loading, setLoading] = useState(false);

  const handleChange = (event) => {
    const newInfo= {...userInfo, [event.target.name]: event.target.value};
  ...
  return (
    <section className='register__form__section' onClick={() => setLoginOpen(false)} >
      <form className='register__form' onSubmit={(target) => {
        ...
        <div className='form__body'>
          ...
          <div className='radio__container'>
            <label>
              <input className='radio__input' type="radio" value="register" checked={role === "register"} onChange={({ target }) => setRole(target.value)} />
              user
            </label>
            <label>
              <input className='radio__input' type="radio" value="register_seller" checked={role === "register_seller"} onChange={({ target }) => setRole(target.value)} />
              seller
            </label>
          </div>
        </div>
